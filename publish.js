...
registerFont('./fonts/Inter-Bold.woff', { family: 'Inter-Bold' })
registerFont('./fonts/Inter-SemiBold.woff', { family: 'Inter-SemiBold' })
registerFont('./fonts/Inter-Medium.woff', { family: 'Inter-Medium' })
...
async function loadSourceImages(data) {
    let loader = source => loadImage(source).then((image) => {
        return image;
    });
    
    return {
        backgroundImage: await loader(`http://localhost/sunset-quality-predictor/publish/resources/instagram/background-${data.rating}.png`)
    }
}
...
async function saveHistory(data) {
    let request = await fetch(`${WEB_SERVER_ROOT}/publish/proxy/saveHistory.php`, {
        method: 'POST',
...
async function postToDestination(params) {
    if (params.destination === 'instagram') {
        ...
        let uploadRequest = await fetch(`${WEB_SERVER_ROOT}/publish/proxy/postPredictionToInstagram.php`, {
            method: 'POST',
